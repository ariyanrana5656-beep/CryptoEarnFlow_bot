...
async def auth(req: AuthReq):
    ...
    if not tg: raise HTTPException(401,'Open from Telegram Mini App only. Send /start to bot first.')
    u=await q('SELECT * FROM users WHERE tg_id=?',(tg['id'],),True)
    if not u:
        uid=await x('INSERT INTO users(tg_id,username,first_name,last_name,photo_url,lang) VALUES(?,?,?,?,?,?)',(tg['id'],tg.get('username',''),tg.get('first_name','User'),tg.get('last_name',''),tg.get('photo_url',''),tg.get('language_code','en')))
        await x('INSERT INTO balances(user_id) VALUES(?)',(uid,)); u=await q('SELECT * FROM users WHERE id=?',(uid,),True)
